...
- [ ] T094 Запретить повторный запуск разбора записи, пока предыдущий не завершён: проверять состояние записи в `startStreamIngest` (`src/worker/routes/streams.ts`) перед записью `processing` и запуском бокса, а журнал бокса вести по имени прогона, чтобы повторный заход не затирал след предыдущего per US2/AC2 (missing)
- [ ] T095 Убрать расшифровку из состояния экземпляра Workflow: шаг `распознать кусок N` возвращает сегменты как результат шага (`src/worker/workflow.ts`), а результаты шагов хранятся в состоянии экземпляра, которое площадка держит три дня, — тогда как расшифровка не должна храниться после разбора per FR-010 (partial)
- [ ] T096 Удалить неиспользуемый `listArchiveVideos` из `src/shared/twitch.ts` — его вытеснил страничный просмотр `listArchive`, вызовов не осталось per Constitution IV (unrequested)
- [ ] T097 Удалить `parseClock` из `src/shared/time.ts` вместе с его проверками в `tests/unit/time.test.ts` — в прикладном коде функция не используется, живёт только собственным тестом per Constitution IV (unrequested)
