...
- [X] T078 Настроить автоматическую публикацию через Workers Builds — после того, как ручной путь заработал (раздел И4 плана)
...
## Phase 9: Запись идущего эфира не берётся в разбор (FR-039)

- [X] T090 Добавить `getLiveStreamId` и поле `streamId` записи в `src/shared/twitch.ts`: площадка заводит запись архива в первые секунды трансляции, и по идентификатору эфира её видно
- [X] T091 Пропускать растущую запись идущего эфира в `selectNextVideo` (`src/worker/schedule.ts`); сбой запроса об идущем эфире валит проверку целиком — принять обрывок за целый эфир хуже, чем пропустить час
- [X] T092 Тесты отбора: растущая запись не берётся, запись законченного эфира берётся, пока идёт другой
- [X] T093 Записать поведение в `spec.md` (FR-039 и сценарий приёмки) и дописать раздел в `research.md`
...
- [X] T094 Запретить повторный запуск разбора записи, пока предыдущий не завершён: проверять состояние записи в `startStreamIngest` (`src/worker/routes/streams.ts`) перед записью `processing` и запуском бокса, а журнал бокса вести по имени прогона, чтобы повторный заход не затирал след предыдущего per US2/AC2 (missing)
- [X] T095 Убрать расшифровку из состояния экземпляра Workflow: шаг `распознать кусок N` возвращает сегменты как результат шага (`src/worker/workflow.ts`), а результаты шагов хранятся в состоянии экземпляра, которое площадка держит три дня, — тогда как расшифровка не должна храниться после разбора per FR-010 (partial)
- [X] T096 Удалить неиспользуемый `listArchiveVideos` из `src/shared/twitch.ts` — его вытеснил страничный просмотр `listArchive`, вызовов не осталось per Constitution IV (unrequested)
- [X] T097 Удалить `parseClock` из `src/shared/time.ts` вместе с его проверками в `tests/unit/time.test.ts` — в прикладном коде функция не используется, живёт только собственным тестом per Constitution IV (unrequested)
