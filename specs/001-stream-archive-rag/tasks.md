...
- [ ] T080 [P] Добавить компоненты shadcn для каркаса: `sidebar` (с ним `sheet`, `tooltip`, `use-mobile`), `breadcrumb` — через `bunx --bun shadcn@latest add` (скилл `shadcn-ui`)
- [ ] T081 Сделать разметку разделов в `src/ui/lib/routes.ts`: адреса, названия, порядок в меню
- [ ] T082 Реализовать переходы между разделами в `src/ui/lib/router.tsx`: History API, обработка «назад», прямая ссылка открывает нужный раздел (FR-035)
- [ ] T083 Собрать каркас в `src/ui/App.tsx`: боковая панель с кнопкой-гамбургером, шапка, область раздела
- [ ] T084 [P] Раздел «Главная» в `src/ui/pages/HomePage.tsx`: что за сервис, как устроен, текущее состояние канала
- [ ] T085 [P] Раздел «MCP» в `src/ui/pages/McpPage.tsx`: переезд инструкции из `components/McpSetup.tsx` (FR-032)
- [ ] T086 [P] Раздел «API» в `src/ui/pages/ApiPage.tsx`: адреса запросов, примеры, коды ошибок — по контракту (FR-036)
- [ ] T087 Раздел «Знания» в `src/ui/pages/KnowledgePage.tsx`: список разобранных и пропущенных, документ по своему адресу (FR-029…FR-031)
- [ ] T088 Раздел «Управление» в `src/ui/pages/ManagePage.tsx`: до верного токена видны только поле токена и пояснение (FR-037)
- [ ] T089 Проверить каркас: `bun run build`, `bun run typecheck`, `bun test`; разделы открываются по прямым адресам, управление скрыто без токена
