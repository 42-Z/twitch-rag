...
- Проход пометки ограничен записями после подключения канала: прошлые в работу не берутся
  (FR-003 спецификации `001`). Практическое следствие — нынешние восемь обрывков в реестр
  не попадут, а причина о трёх минутах появится в списке на первом неудавшемся запуске
  эфира после выпуска. Записано в «Assumptions» спецификации.
